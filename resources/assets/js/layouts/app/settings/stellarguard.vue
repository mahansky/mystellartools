                ...
                <v-card-text>
                    <v-layout>
                        ...
                        <v-flex xs10>
                            ...
                            <v-btn v-if="!guarded && !badNetwork && step === 0" info class="mx-0" @click="step = 1"><v-icon class="pr-2">security</v-icon> Enable protection</v-btn>
...
// import { getAccount } from '~/stellar/stellarguard'
...
export default {
  ...
  computed: {
    registerLink() {
      return this.$store.getters.transactionsNetwork.type === "main"
  ...
  created() {
    if (this.$store.getters.transactionsNetwork.type === "main") {
      StellarGuardSdk.usePublicNetwork();
    } else if (this.$store.getters.transactionsNetwork.type === "testnet") {
    ...
    //console.log(getAccount(this.$store.getters.keypair.publicKey()))
  }
};
</script>
